...
Route::any('/search',function(){
    ...
    $q = Request::get ( 'q' );
    $result = DB::table('customers', 'groups')->select('customers.id', 'image_name', 'surname', 'middle_name', 'first_name','aka','name', 'groups.id as groupid', 'groups.user_id')->join('groups', 'groups.id', '=', 'customers.group_id')->where('surname','LIKE','%'.$q.'%')->orWhere('middle_name','LIKE','%'.$q.'%')->orWhere('first_name','LIKE','%'.$q.'%')->get();
    if(count($result) > 0)
